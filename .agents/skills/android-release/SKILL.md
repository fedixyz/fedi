...
Where the notes text comes from is the user's call, so ask: a feature release usually wants new copy, a patch often reuses the current production notes. When new copy is needed and none exists anywhere yet, the notes have not been distilled from the release contents: run the `report-next-release` skill, draft the copy from its summary cards by the rules in its `references/release-notes-copy.md`, and get the user's sign-off before writing any locale. Translate the signed-off English into every other locale yourself, keeping the previous release's notes in each language as the reference for register and recurring terms. No translation service or API key is involved. The Play locale set is its own thing (`es-419` and `sw` exist here and not on the App Store), so port notes store-to-store by locale mapping the user confirms, not by assumption.
...
End the report with the link to that page, not a pointer to the console: `https://play.google.com/console/u/0/developers/5515229017025140225/app/4975438750291334651/tracks/production?tab=releases` is the production releases page of `com.fedi`, where the committed draft sits with its Review release button. The number after `app/` is the console's own app id, read off the console URL since the publishing API only knows package names (`com.fedi.nightly` is 4975347406864887167, `com.fedi.edge` is 4974544935160849695).
...
- the draft's body is where the release notes land before publishing. A published release is the strongest in-repo signal that a version actually shipped, and its body is the internal record of what went out, so a body still carrying only the commit line means the release notes have not been produced yet. Distill them by the `report-next-release` skill's `references/release-notes-copy.md`, which also fixes the body's shape, get the user's sign-off, then stage them with `gh release edit <version> --notes-file <file>`, which finds drafts by tag and changes nothing visible. A feature release also gets the rendered report attached as HTML and PDF, `gh release upload <version> <html> <pdf>`, which is just as invisible until publish. Never pass `--draft=false`, which is the publish switch.
...
End the report with the draft's own link, `gh release view 26.X.Y --repo fedibtc/fedi --json url --jq .url`, read last because GitHub changes a draft's URL on every edit. Put the tag URL next to it, `https://github.com/fedibtc/fedi/releases/tag/26.X.Y`, which becomes valid once the draft is published.
