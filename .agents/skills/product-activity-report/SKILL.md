...
Four things about this repo's release model make the naive version of this check wrong, and the script exists because each one silently produces a confident false answer.

**Nothing deploys itself, so the newest tag is not what users are running.** Web production is a manual `workflow_dispatch` of `vercel-prod.yml`, and the `Production` environment only accepts a `web/*` ref of type tag, so a branch is rejected before any build runs. A `web/26.7.0` tag can therefore sit in the repo indefinitely without a single user seeing it. The deployed ref is whichever one the last *successful* production run used, and the native equivalent is the newest published GitHub release, read through its build record rather than its tag. The script asks the deploy rather than the tag list, and when it has to fall back to newest-tag it labels that ref `NOT confirmed deployed` so a guess never gets reported as fact.

**A native release tag is cut at publish time, so it can name a commit that never shipped.** The release workflows build the APK and open the GitHub release as a draft. A draft has no tag. GitHub creates one only when a person publishes, at the commit the draft targets, and before the `target_commitish` pin that target was master's head at publish time. `26.8.2` was built from `release/26.8` and published a day later, so its tag names a master commit no user ever ran, and ancestry against that tag reports every master commit before it as live. The `Built from commit:` line in the body comes from the build, so the script resolves that first and uses the tag only when the two agree. Its provenance line says which one it used.
