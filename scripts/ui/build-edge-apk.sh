...
echo "Building edge flavor release APK with fastlane (see $REPO_ROOT/ui/native/android/Fastfile for lane configurations)..."
fastlane build_edge_apk

RELEASE_PATH="$REPO_ROOT/ui/native/android/app/build/outputs/apk/edge/release"
echo "APK built successfully at $RELEASE_PATH/app-edge-release.apk"

if [ -n "$APK_PATH" ]; then
  SOURCE="$RELEASE_PATH/app-edge-release.apk"
