...
type ScreenResult =
    | { screen: RootScreen }
    | { screen: TabScreen; parent: 'TabsNavigator' }
...
> = {
    ...
    chat: (params: Record<string, string>) => {
        if (params?.userId) return { screen: 'ChatUserConversation' }
        if (params?.roomId) return { screen: 'ChatRoomConversation' }
        return { screen: 'Chat', parent: 'TabsNavigator' }
    },
...
export function normalizeDeepLink(urlString: string) {
    try {
        ...
        log.error('normalizeDeepLink error', err)
        return undefined
    }
}
